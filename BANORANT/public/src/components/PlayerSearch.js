...
export function renderPlayerSearch(container) {
  container.innerHTML = `
    <div class="glass mb-6">
      ...
      <div class="flex flex-col md:flex-row items-center gap-2">
        <input id="player-name-input" type="text" placeholder="Player Name (case-sensitive, e.g. Takoyaki)" class="px-4 py-2 rounded bg-gray-800 text-white flex-1" />
        <input id="player-tag-input" type="text" placeholder="Tag (e.g. VINCE)" class="px-4 py-2 rounded bg-gray-800 text-white flex-1" />
        <input id="api-key-input" type="password" placeholder="API Key" class="px-4 py-2 rounded bg-gray-800 text-white flex-1" />
  ...
  searchBtn.onclick = async () => {
    ...
    if (!name || !tag || !apiKey) {
      resultDiv.innerHTML = `<div class="text-red-400">Please enter name, tag, and API key.</div>`;
      return;
    }
    
    resultDiv.innerHTML = `<div class="text-gray-400">Searching...</div>`;
    try {
      const player = await searchPlayer(name, tag, apiKey); 
      resultDiv.innerHTML = `
